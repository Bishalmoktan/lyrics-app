...
const SongCard = ({ song, showDetails = true } : SongCardProps) => {
  const { setCurrentSongId, isPlaying, currentSongId, handlePlayPause } = useGlobalApp();
  ...
  return (
    <div className="flex items-center bg-[#1A2A44] p-4 rounded-lg hover:bg-[#243656] transition-colors">
      ...
      <div>
        <h3 className={cn("font-semibold line-clamp-1", isThisSongPlaying && 'text-rose-500')}>{song.title}</h3>
      {showDetails &&  <p className="text-sm text-gray-400 line-clamp-1">{song?.Artist?.name} • {song?.Artist?.designation}</p>}
      </div>
